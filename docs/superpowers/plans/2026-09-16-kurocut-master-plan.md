...
```mermaid
flowchart TD
    S[yt-dlp 抓取源视频] --> T[FFmpeg 简单裁切测试片段]
    T --> A[完整日语回放或保留源文件]
    A --> B[本地转录：日语与源时间戳]
    A --> C[全场声音线索与低频画面观察]
    B --> D[合并候选：不只选响声大的地方]
...
- [ ] 用 yt-dlp 抓取 A 场，并用 FFmpeg 从副本裁出 60–90 秒小样及一个 10–20 分钟区间，供安装与基础功能测试；不要修改原文件。
...
- [x] 运行原有 UI/命令，先处理 60–90 秒日语小样，检查文字、时间戳及带字幕导出。
- [ ] 用 10–20 分钟回放测试粗剪；先允许人工给时间段，以排除选段质量对底层验证的干扰。
- [x] 本地模式运行，无 API 密钥不影响本阶段。不要为跑通示例自动注册服务、充值或上传素材。
